...
impl Kid {
    ...
    pub fn parse_row(csv_row: &str) -> Result<Kid, ParseError> {
        ...
        let good_deeds_string = fields
            .next()
            .ok_or(ParseError::NoGoodDeeds)?;
        if good_deeds_string.is_empty() {
            return Err(ParseError::NoGoodDeeds);
        }
        let good_deeds = good_deeds_string
            .parse::<u32>()
            .map_err(|_| ParseError::InvalidGoodDeeds)?;

        let bad_deeds_string = fields
            .next()
            .ok_or(ParseError::NoBadDeeds)?;
        if bad_deeds_string.is_empty() {
            return Err(ParseError::NoBadDeeds);
        }
        let bad_deeds = bad_deeds_string
            .parse::<u32>()
